...
| [`example/auth-supabase`](example/auth-supabase/) | The same seam for Supabase Auth, and the authentication half of [running sqlb on Supabase](docs/supabase.md). It refuses a project's `anon` and `service_role` keys, which are JWTs that project signed and published |
