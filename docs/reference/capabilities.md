...
`unindexed-filter` and `unindexed-sort` say the scan costs one tenant's rows
rather than the table, and the index they name leads with the scope column,
because the predicate is never the filtered column alone. A table keyed by its
own scope has one row per caller, so neither rule fires on it at all.
